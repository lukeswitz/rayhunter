...
pub struct RecordingStore {
    ...
    manifest_file_mux: Mutex<()>, // Prevent concurrent writes to manifest.toml
}
...
impl RecordingStore {
    ...
    async fn write_manifest(&mut self) -> Result<(), RecordingStoreError> {
        // Acquire manifest_file_mux to prevent concurrent file writes.
        let _lock = self.manifest_file_mux.lock().await;
        let mut manifest_file = File::create(self.path.join("manifest.toml"))
            .await
            .map_err(RecordingStoreError::WriteManifestError)?;
